...
class MovieMapperTest : BaseDataTestClass() {
    ...
    fun test_TransformPropertyDto() {
        ...
        assertThat<Double>(
            properties?.get(0)?.voteAverage,
            Is.`is`<Double>(dummyMovieDto.voteAverage)
        )
        assertThat<String>(properties?.get(0)?.title, Is.`is`<String>(dummyMovieDto.title))
        assertThat<Double>(
            properties?.get(0)?.popularity,
            Is.`is`<Double>(dummyMovieDto.popularity)
        )
        assertThat<String>(
            properties?.get(0)?.posterPath,
            Is.`is`<String>(dummyMovieDto.posterPath)
        )
        assertThat<String>(
        ...
        assertThat<Int>(
            properties?.get(0)?.genreIds?.size,
            Is.`is`<Int>(dummyMovieDto.genreIds.size)
        )
        assertThat<String>(
            properties?.get(0)?.backdropPath,
            Is.`is`<String>(dummyMovieDto.backdropPath)
        )
        ...
        assertThat<String>(
            properties?.get(0)?.releaseDate,
            Is.`is`<String>(dummyMovieDto.releaseDate)
        )
