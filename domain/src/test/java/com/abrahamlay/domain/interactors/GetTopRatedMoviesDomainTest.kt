...
class GetTopRatedMoviesDomainTest : BaseDomainTestClass() {

    // SUT
    private lateinit var interactor: GetTopRatedMovies
    ...
    fun setUp() {
        interactor = GetTopRatedMovies(repository, mockPostExecutionThread)
    }

    @Test
    fun test_GivenAPIKey_WhenGetTopRatedMoviesExecuted() {
        val params = GetTopRatedMovies.Params(API_KEY)
        BDDMockito.given(repository.getTopRatedMovies(API_KEY)).willReturn(mockFlowable)
